...
function IGRF13(r_eci, epc::SatelliteDynamics.Epoch)
    ...
    # get decimal date
    date = SatelliteDynamics.caldate(epc)
    year = date[1]
    day = SatelliteDynamics.day_of_year(epc)
    decimal_date = year + day / 365.2425

    # println("(jl) Date: $decimal_date")

    # eci and ecef location
    ecef_Q_eci = SatelliteDynamics.rECItoECEF(epc)
...
# See Markley and Crassidis pg 403

earth_magnetic_dipole = let
    a_mag = 6371.2e3 # magnetic spherical reference radius
    g11 = -1669.05e-9 # T
    h11 = 5077.99e-9 # T
    g01 = -29554.63e-9 # T
    a_mag^3 * [g11, h11, g01] # eq D.4
end

dipole_magnetic_potential_V(r) = (earth_magnetic_dipole'r) / (sqrt(r'r)^3) # eq D.3

dipole_magnetic_field_B(r) = -ForwardDiff.gradient(dipole_magnetic_potential_V, r) # should be in Tesla for torque to be Nm